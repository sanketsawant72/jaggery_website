...
function handleNavSearch() {
  ...
  var results = products.filter(function(p) { return p.name.toLowerCase().includes(q); });
  var grid = document.getElementById('productsContainer');
  if (!grid) return;

  if (!results.length) {
    grid.innerHTML = '<p style="color:var(--muted);padding:2rem;text-align:center;">No products found for "' + input.value + '"</p>';
    return;
  }
